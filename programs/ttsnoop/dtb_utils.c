...
)
{
    ...
    static Widget	MoreButton;Boolean _DtCanHelp( const char * );
	...
	*/
    ...
    XtSetSensitive(MoreButton,_DtCanHelp(help_data->help_text));XtManageChild(Quick_help_dialog);
}
...
)
{
    ...
    */
    if (_DtHelped( help_dialog)) return;if( (cp=strrchr(buffer,'/')) != (char *)NULL) {
	*cp++ = 0;
	vol = buffer;
	loc = cp; 
    }
